...
from nmr_rise.mol2nmr.utils import parse_select_atom, TargetScaler
from ase import Atoms
logger = logging.getLogger(__name__)
from tqdm import tqdm

def process_dataset(dataset, dictionary, args):
    ...
    select_atom_dataset = SelectTokenDataset(token_dataset=token_dataset, token_mask_dataset=atoms_target_mask_dataset, selected_token=parse_select_atom(dictionary, args.selected_atom))
...
@register_task("mol2nmr")
class Mol2NMRTask(UnicoreTask):
    ...
    def __init__(self, args, dictionary):
        ...
        if self.args.split_mode =='predefine':
            self.train_dataset = HuggingFaceDataset(self.args.data, split='train')
            self.train_dataset = process_dataset(self.train_dataset, self.dictionary, self.args)

            self.valid_dataset = HuggingFaceDataset(self.args.data, split='validation')
            self.valid_dataset = process_dataset(self.valid_dataset, self.dictionary, self.args)
            ...
            self.test_dataset = HuggingFaceDataset(self.args.data, split='test')
            self.test_dataset = process_dataset(self.test_dataset, self.dictionary, self.args)
        else:
            self.__init_data()
