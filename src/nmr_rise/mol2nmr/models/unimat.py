...
class GaussianLayer(nn.Module):
    ...
    def forward(self, x, edge_type):  # x: (atoms, hidden),  edge_type: atoms, atoms
        if (edge_type < 0).any():
            raise ValueError('Negative indices found in edge_type')
        max_idx = edge_type.max().item()
        if max_idx >= self.mul.num_embeddings:
            raise ValueError(f'edge_type max index {max_idx} out of range (max allowed: {self.mul.num_embeddings-1})')
